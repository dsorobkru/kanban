...
(function () {
  this.jKanban = function () {
    ...
    this.states = ['Назначена', 'В работе', 'На паузе', 'Согласования', 'В ожидании', 'Выполнена'];
    this.boardsColor = '3F6088';
    this.boardsCount = 0;
    var defaults = {
      element: '',
      gutter: '15px',
    ...
    this.setDragBoards = function(status) {
      this.options.dragBoards = status;
    }
    ...
    this.getAllBoardsHeaders = function () {
      var els = self.element.getElementsByClassName("kanban-board-header board-head")
      return els
    }

    this.deleteAllBoards = function() {
      var allBoards = this.getAllBoards();
      var allBoards = Array.prototype.slice.call(allBoards);
      for (let i=0; i < allBoards.length; i++) {
        var curBoard = allBoards[i];

        var deleteBoardId = curBoard.getAttribute('data-id');

        this.removeBoard(deleteBoardId);
      }
    }

    this.addDeleteButtonInAllHeaders = function () {
      var oldElemetnsCollections = document.getElementsByClassName('board-delete-button');  
      var oldElemetns = Array.prototype.slice.call(oldElemetnsCollections);
      for (let i=0; i < oldElemetns.length; i++) {
        var elem = oldElemetns[i];
        elem.parentNode.removeChild(elem);
      }

      var oldElemetnsCollections = document.getElementsByClassName('board-empty-button');  
      var oldElemetns = Array.prototype.slice.call(oldElemetnsCollections);
      for (let i=0; i < oldElemetns.length; i++) {
        var elem = oldElemetns[i];
        elem.parentNode.removeChild(elem);
      ...
      var allBoardsHeaders = this.getAllBoardsHeaders();

      for (let i=0; i < allBoardsHeaders.length; i++) {
        var boardHeader = allBoardsHeaders[i];
        var boardId = boardHeader.parentElement.getAttribute('data-id');

        boardHeader.insertAdjacentHTML('beforeend', '<input type="button" class="board-delete-button" id="delete_button' + boardId + '" value=&#11198;>');
        boardHeader.insertAdjacentHTML('afterbegin', '<input type="button" class="board-empty-button" value=&#11198;>');
      }

      function deleteBoard(e) {
        var re = /delete_button/gi;
        var deleteButtonId = e.srcElement.getAttribute('id');
        
        var deletedBoardId = deleteButtonId.replace(re, "");

        KanbanTest.removeBoard(deletedBoardId);

        KanbanTest.addDeleteButtonInAllHeaders();
        KanbanTest.addAddBoardButtons();
        KanbanTest.addSelectsEvent();
        KanbanTest.addScroll();
        KanbanTest.changeBoardsColor(null, true);
        KanbanTest.changeBoardsWidth(null);

        document.getElementsByClassName('kanban-container')[0].style.width = "100%";

        if (KanbanTest.getAllBoards().length == undefined) {
          KanbanTest.deleteScroll();
        }
      }

      var deleteButtons = document.getElementsByClassName('board-delete-button');
      for (let i=0; i < deleteButtons.length; i++) {
        deleteButtons[i].addEventListener("click", deleteBoard);
      }
    }

    this.getTitleStates = function() {
      var titleStates = '';
      for (let i=0; i<KanbanTest.states.length; i++) {
        titleStates += '<option states class="states-option">' + KanbanTest.states[i] + '</option>';
      }

      titleStates = '<select states class="states-select">' + titleStates + '</select>';

      return titleStates;
    }

    this.fixBoardsId = function() {
      ...
      for (let i=0; i < selects.length; i++) {
        ...
        curBoard.setAttribute('data-id', this.getIdBySelectText(selectText));
      } 
    }

    this.fixSelects = function() {
      var boards = this.getAllBoards();
      for (let i=0; i < boards.length; i++) {
        var curBoard = boards[i];
        var boardId = curBoard.getAttribute('data-id');
        var curSelect = curBoard.getElementsByClassName('states-select')[0];
        curSelect.value = this.getSelectTextById(boardId);
      }
    }

    this.getSelectTextById = function(boardId) {
      var newText = '';
    ...
    this.getIdBySelectText = function(selectText) {
      var newId = '';
    ...
    this.addSelectsEvent = function() {     
      function changeBoardId(e) {
        var curSelect = e.srcElement;
        var selectText = curSelect.options[curSelect.selectedIndex].text;
        var curBoard = curSelect.parentElement.parentElement.parentElement;
        
        curBoard.setAttribute('data-id', KanbanTest.getIdBySelectText(selectText));
      }

      var selects = document.getElementsByClassName("states-select");
      for (let i=0; i < selects.length; i++) {
        curSelect = selects[i];
        curSelect.addEventListener('change', changeBoardId);
      }
    }

    this.checkBoardsRepeat = function() {
      var result = 'Ok';
      ...
      if (_appointed >= 2) {
        result = 'Слишком много статусов Назначена';
      } else if (_inwork >= 2) {
        result = 'Слишком много статусов В работе';
      } else if (_pause >= 2) {
        result = 'Слишком много статусов На паузе';
      } else if (_agreements >= 2) {
        result = 'Слишком много статусов Согласования';
      } else if (_pending >= 2) {
        result = 'Слишком много статусов В ожидании';
      } else if (_completed >= 2) {
        result = 'Слишком много статусов Выполнена';
      }

      return result;
    }

    this.getDragToBoards = function(boardId) {
      var dragTo = [];

      switch(boardId) {
        case '_appointed':
          dragTo = ['_inwork', '_agreements'];
          break;
        case '_inwork':
          dragTo = ['_pause', '_pending', '_completed'];
          break;
        case '_pause':
          dragTo = ['_inwork', '_agreements', '_pending'];
          break;
        case '_agreements':
          dragTo = ['_inwork'];
          break;
    ...
    this.deleteScroll = function() {
      var oldLeft = document.getElementById('scroll-left');
      if (oldLeft != undefined) {
        oldLeft.parentNode.removeChild(oldLeft);
      }

      var oldRight = document.getElementById('scroll-right');
    ...
    this.addScroll = function() {
      ...
      document.getElementById('myKanban').insertAdjacentHTML('afterbegin', '<div id="scroll-left"></div>');
      document.getElementById('myKanban').insertAdjacentHTML('beforeend', '<div id="scroll-right"></div>');


      var kanbanContainer = document.getElementsByClassName('kanban-container')[0];
    ...
    this.changeBoardsColor = function(color, creater) {
      if (color == null) {
        color = this.boardsColor;
      }

      this.boardsColor = color;

      var textColor = this.pickTextColorBasedOnBgColorAdvanced('#' + color, '#FFFFFF', '#000000');
      var boardHeaders = KanbanTest.getAllBoardsHeaders();
      for (var i = 0; i < boardHeaders.length; i++) {
        var boardHeader = boardHeaders[i];
        boardHeader.style.background = '#' + color;
        boardHeader.style.color = textColor;
  
        if (creater) {
          var selectHeader = boardHeader.getElementsByClassName('states-select')[0];
          selectHeader.style.backgroundColor = '#' + color;
          selectHeader.style.color = textColor;

          var deleteButton = boardHeader.getElementsByClassName('board-delete-button')[0];
          deleteButton.style.color = textColor;
  
          var optionsSelect = selectHeader.getElementsByClassName('states-option');
          for (j=0; j < optionsSelect.length; j++) {
            var optionSelect = optionsSelect[j];
            optionSelect.style.backgroundColor = '#' + color;
            optionSelect.style.color = textColor;
          }
        } 
      }
    }
    
    this.pickTextColorBasedOnBgColorAdvanced = function(bgColor, lightColor, darkColor) {
    ...
    this.addMarginToBoards = function() {
      var allBoards = this.getAllBoards();
      for (let i=0; i < allBoards.length; i++) {
        var curBoard = allBoards[i];
        curBoard.style.marginLeft = '0.5%';
        curBoard.style.marginRight = '0.5%';
      }
    }

    this.addAddBoardButtons = function () {
      var oldElemetnsCollections = document.getElementsByClassName('add-board-button');  
      var oldElemetns = Array.prototype.slice.call(oldElemetnsCollections);
      for (let i=0; i < oldElemetns.length; i++) {
        var elem = oldElemetns[i];
        elem.parentNode.removeChild(elem);
      ...
      document.getElementsByClassName('kanban-container')[0].insertAdjacentHTML('afterbegin', '<div class="add-board-button" id="add-new-board_0"><input class="input-add-button" style="display: block;" type="button" value="+"></div>');
      
      for (let i=0; i < allBoards.length; i++) {
        var board = allBoards[i];
        var boardId = board.getAttribute('data-id');

        board.insertAdjacentHTML('afterend', '<div class="add-board-button" id="add-new-board' + boardId + '"><input class="input-add-button" type="button" value="+"></div>');
      }

      function overButton(e) {
        var addButton = e.srcElement;
        var inputButton = addButton.getElementsByClassName('input-add-button')[0];
        if (inputButton != undefined) { 
          inputButton.style.display = 'block';
          addButton.style.width = '2.5%';
          addButton.style.minWidth = '2.5%';
        }
      }
      ...
      function outButton(e) {
        ...
        if (inputButton != undefined) {
          ...
          addButton.style.minWidth = '1%';
        }
      }

      function overInput(e) {
        var inputButton = e.srcElement;
        var addButton = inputButton.parentElement;
        if (inputButton != undefined) { 
          inputButton.style.display = 'block';
          addButton.style.width = '2.5%';
          addButton.style.minWidth = '2.5%';
      ...
      var numeratic = 1;
      var allAddButtons = document.getElementsByClassName('add-board-button');
      var firstAddButton = document.getElementById('add-new-board_0');
      var firstAddInput = firstAddButton.getElementsByClassName('input-add-button')[0];

      if (allAddButtons.length != 1) {
        firstAddInput.style.display = null;

        numeratic = 0;
      }
      for (let i=numeratic; i < allAddButtons.length; i++) {
        allAddButtons[i].addEventListener("mouseover", overButton);
        allAddButtons[i].addEventListener("mouseout", outButton);
      }

      var allAddInputs = document.getElementsByClassName('input-add-button');
      for (let i=numeratic; i < allAddInputs.length; i++) {
        allAddInputs[i].addEventListener("mouseover", overInput);
        allAddInputs[i].addEventListener("mouseout", outInput);
      }

      function addNewBoard(e) {
        var re = /add-new-board/gi;
        var addButtonId = e.srcElement.parentElement.getAttribute('id');
        
        var addAfterBoardId = addButtonId.replace(re, "");
        
        var secondBoard = e.srcElement.parentElement.nextElementSibling;
        var newId = '_null' + addAfterBoardId;
        ...
        var newBoard = KanbanTest.findBoard(newId);
        var kanbanContainer = document.getElementsByClassName('kanban-container')[0];

        newBoard.parentElement.insertBefore(newBoard, secondBoard);
      ...
      var addBoardButtonsButtons = document.getElementsByClassName('add-board-button');
      for (let i=0; i < addBoardButtonsButtons.length; i++) {
        addBoardButtonsButtons[i].addEventListener("click", addNewBoard);
      }
    }

    this.changeBoardsWidth = function(count) {
      if (count == null) {
        count = this.boardsCount;
      }

      this.boardsCount = count;
      ...
      var BoardWidht = this.getBoardWidth(count);

      for (let i=0; i < boards.length; i++) {
        var curBoard = boards[i];
        curBoard.style.width = BoardWidht;
        curBoard.style.minWidth = BoardWidht;
      }
    }

    this.getBoardWidth = function(count) {
      var width = "24%";
